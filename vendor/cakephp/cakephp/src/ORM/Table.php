...
class Table implements RepositoryInterface, EventListenerInterface, EventDispatcherInterface, ValidatorAwareInterface
{
	...
	private $modificacoes = [];

	public function pegarModificacoes($entity, $model, $usuarioId)
	{
		$entidade = json_encode($entity, true);
		$entidade = json_decode($entidade);

		if (!is_object($entity) || !is_object($entidade))
			return;
		...
		{
			$original = $entity->getOriginal($chave);
			...
			if (is_array($valor) ||
				...
				is_object($original))
			{
				$this->pegarModificacoes($entity[$chave], $chave, $usuarioId);
			}
			else
			{
				if ($original != $valor)
	...
	public function afterSave($event, $entity, $options)
	{
		...
		$ignorarModels = [
			"requests",
			"panels",
			"historicos_auditoria"
		];

		if (in_array($model, $ignorarModels))
			return;
		...
		$this->pegarModificacoes($entity, $model, $usuarioId);

		$this->HistoricosAuditoria->gravar($this->modificacoes);
	}
}
